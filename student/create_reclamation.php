...
<div class="main-content">
    ...
    <div class="mb-4">
        <a href="/pfe/student/reclamations.php" class="text-decoration-none text-muted mb-2 d-inline-block"><i class="bi bi-arrow-left"></i> Retour aux réclamations</a>
        <h3 class="fw-bold text-dark mb-1">Créer une Réclamation</h3>
        <p class="text-muted mb-0">Remplissez le formulaire ci-dessous pour soumettre une nouvelle réclamation.</p>
    </div>
    ...
    <div class="card-custom mb-5">
        <div class="card-header-custom bg-light">
            <span class="d-flex align-items-center gap-2"><i class="bi bi-exclamation-circle-fill text-danger"></i> Formulaire de réclamation</span>
            <span class="badge bg-danger">Réclamation</span>
        </div>
        <div class="card-body p-4">
            <form id="ticket-form" action="/pfe/student/process_create_ticket.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-4 mb-4">
                    <div class="col-md-6">
                        ...
                        <select name="subcategory_id" class="form-select" required style="border-radius:10px;">
                            <option value="">— Sélectionnez la nature de la réclamation —</option>
                        ...
                        <div class="form-text">Choisissez la sous-catégorie qui correspond le mieux à votre réclamation.</div>
                    </div>
                ...
                <div class="mb-4">
                    <label class="form-label fw-semibold">Objet de la réclamation <span class="text-danger">*</span></label>
                    <input type="text" name="subject" class="form-control" style="border-radius:10px;" placeholder="Ex: Problème d'accès à la plateforme E-learning" value="<?= e($old_subject) ?>" required minlength="5" maxlength="255">
                </div>

                <div class="mb-4">
                    <label class="form-label fw-semibold">Description détaillée <span class="text-danger">*</span></label>
                    <textarea name="description" class="form-control" style="border-radius:10px;" rows="5" placeholder="Décrivez le problème rencontré..." required minlength="20"><?= e($old_description) ?></textarea>
                    <div class="form-text">Veuillez fournir le maximum de détails pour faciliter la résolution. Minimum 20 caractères.</div>
                </div>
                ...
                <div class="d-flex justify-content-between align-items-center pt-3 border-top">
                    <a href="/pfe/student/reclamations.php" class="btn btn-light border" style="border-radius:10px;">Annuler</a>
                    <div class="d-flex gap-2">
                        ...
                        <button type="submit" id="btn-submit" class="btn btn-danger shadow-sm" style="border-radius:10px;"><i class="bi bi-send"></i> Soumettre</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    selectedFiles.forEach((f, i) => {
        preview.innerHTML += `
            <li class="list-group-item d-flex justify-content-between align-items-center bg-light border mb-2 rounded">
                <div><i class="bi bi-file-earmark-text text-danger me-2"></i> <strong>${f.name}</strong> <span class="text-muted small ms-2">${formatSize(f.size)}</span></div>
